...
window.getCountryCode = function() {
    ...
    switch (getStore()) {
        ...
        case store.Nederland: return "NL";
...
window.getCountryLanguageCode = function(){
    switch (getStore()) {
        ...
        case store.Australia: return "en-AU";
        case store.Nederland: return "nl";
...
// add pobox for us store
if (getStore() == store.UnitedStates)
{
    // add pobox message label, default is hide
...
for (i = 0; i < fields.length; i++) document.getElementById(fields[i]).maxLength = Max_Length;
...
//var maxLengthMessage_Fr = " caractères restants (" + Max_Length + " maximum)";

// todo - translate 35 characters remaining (35 maximum)
function getMaxLengthMessage(){
    switch (getStore()) {
        ...
        case store.Nederland: return " characters remaining (" + Max_Length + " maximum)";
...
// todo - translate the max length of 35 characters is reached
function getMaxLengthReachMessage(){
    switch (getStore()) {
        ...
        case store.Nederland: return "the max length of <b>" + Max_Length + "</b> characters is reached";
...
// add pobox event, it should be added after modify the address control
var select = document.getElementById("checkout_shipping_address_country");
var country = select.options[select.selectedIndex].value;
